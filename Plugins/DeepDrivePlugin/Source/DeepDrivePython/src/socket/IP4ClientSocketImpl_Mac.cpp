...
IP4ClientSocketImpl_Mac::IP4ClientSocketImpl_Mac()
{
...
bool IP4ClientSocketImpl_Mac::connect(const IP4Address &ip4Address)
{
	if(!m_isConnected)
	{
...
int32 IP4ClientSocketImpl_Mac::send(const void *data, uint32 bytesToSend)
{
...
int32 IP4ClientSocketImpl_Mac::receive(void *buffer, uint32 size)
{
	int32 res = ClientErrorCode::NOT_CONNECTED;
	if(m_isConnected)
	{
...
int32 IP4ClientSocketImpl_Mac::receive(void *buffer, uint32 size, uint32 timeOutMS)
{
	int32 res = ClientErrorCode::NOT_CONNECTED;
...
void IP4ClientSocketImpl_Mac::close()
{
	if(m_isConnected)
	{
...
bool IP4ClientSocketImpl_Mac::isConnected() const
{
	return m_isConnected;
}
