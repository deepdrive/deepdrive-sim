...
int32 DeepDriveSimulation::initialize(DeepDriveClient &client, uint32 seed, float timeDilation, float startLocation)
{
	deepdrive::server::InitializeSimulationRequest	req(client.getClientId(), seed, timeDilation, startLocation);
	IP4ClientSocket &socket = client.getSocket();

	int32 res = socket.send(&req, sizeof(req));
	if(res >= 0)
	{
		std::cout << "InitializeSimulationRequest sent\n";

		deepdrive::server::InitializeSimulationResponse response;
		if(socket.receive(&response, sizeof(response), 1000))
		{
			...
			std::cout << "Waiting for InitializeSimulationRequest, time out\n";
	}
