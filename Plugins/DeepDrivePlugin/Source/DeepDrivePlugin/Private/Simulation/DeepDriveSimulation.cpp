...
void ADeepDriveSimulation::PreInitializeComponents() {
    ...
    for (auto &actor : actors) {
        ...
        if (simu
            ...
            if (simu->isActive()) {
                ...
                UE_LOG(LogDeepDriveSimulation, Log, TEXT("Another Simulation [%s] is already registered"),
                       *(simu->GetFullName()));
                break;
            }
        }
    ...
    if (!alreadyRegistered) {
        ...
        if (m_ServerProxy && m_ServerProxy->initialize(ClientsIPAddress, ClientsPort, GetWorld())
            ...
            UE_LOG(LogDeepDriveSimulation, Error,
                   TEXT("DeepDriveSimulation [%s] could not be activated ServerProxy %p"), *(GetFullName()),
                   m_ServerProxy);
        }
    }
            ...
            if (m_curAgent) {
                ...
                if (CameraType == EDeepDriveAgentCameraType::FREE_CAMERA) {
                    if (FreeCamera) {
                        ...
                        FreeCamera->SetActorTransform(FTransform(agentTransform.Rotator(),
                                                                 agentTransform.GetLocation() +
                                                                 FVector(0.0f, 0.0f, 200.0f),
                                                                 agentTransform.GetScale3D()));
                        camActor = FreeCamera;
                    }
                } else {
                    camActor = m_curAgent;
        ...
        if (m_curAgent) {
            ...
            if (type == EDeepDriveAgentCameraType::FREE_CAMERA) {
                if (FreeCamera) {
                    FTransform agentTransform = m_curAgent->GetActorTransform();
                    FreeCamera->SetActorTransform(FTransform(agentTransform.Rotator(),
                                                             agentTransform.GetLocation() + FVector(0.0f, 0.0f, 200.0f),
                                                             agentTransform.GetScale3D()));
                    camActor = FreeCamera;
                }
            } else {
...
void ADeepDriveSimulation::SelectMode(EDeepDriveAgentControlMode Mode) {
    if (Mode != m_curAgentMode) {
        ...
        UE_LOG(LogDeepDriveSimulation, Log, TEXT("SelectMode activating new mode: %s"),
               *(pEnum ? pEnum->GetNameByIndex(static_cast<uint8>(Mode)).ToString() : FString("UNKOWN")));

        ADeepDriveAgentControllerBase *controller = spawnController(Mode, 0, 0);
        ADeepDriveAgentControllerBase *prevController = Cast<ADeepDriveAgentControllerBase>(
                m_curAgent->GetController());

        if (controller && controller->Activate(*m_curAgent, true)) {
            if (prevController) {
...
void ADeepDriveSimulation::applyGraphicsSettings(const SimulationGraphicsSettings &gfxSettings) {
    ...
    if (gameSettings) {
        gameSettings->SetFullscreenMode(
                gfxSettings.is_fullscreen > 0 ? EWindowMode::WindowedFullscreen : EWindowMode::Windowed);
        ...
        UE_LOG(LogDeepDriveSimulation, Log, TEXT("DeepDriveSimulation Texture Quality %d"),
               gfxSettings.texture_quality);
...
void ADeepDriveSimulation::removeAgents(bool removeEgo) {
    for (signed i = removeEgo ? 0 : 1; i < m_Agents.Num(); ++i) {
        ADeepDriveAgentControllerBase *agentController = Cast<ADeepDriveAgentControllerBase>(
                m_Agents[i]->GetController());
...
ADeepDriveAgent *
ADeepDriveSimulation::spawnAgent(const FDeepDriveAgentScenarioConfiguration &scenarioCfg, bool remotelyControlled) {
    ...
    if (agent) {
        ...
        EDeepDriveAgentControlMode mode = remotelyControlled ? EDeepDriveAgentControlMode::REMOTE_AI
                                                             : EDeepDriveAgentControlMode::CITY_AI;
        if (ControllerCreators.Contains(mode)) {
            ADeepDriveAgentControllerBase *controller = ControllerCreators[mode]->CreateAgentControllerForScenario(
                    scenarioCfg, this);
            if (controller) {
...
ADeepDriveAgent *
ADeepDriveSimulation::spawnAgent(EDeepDriveAgentControlMode mode, int32 configSlot, int32 startPosSlot) {
    UE_LOG(LogDeepDriveSimulation, Log, TEXT("Try to spawn agent config slot %d startPosSlot %d"), configSlot,
           startPosSlot);
    ...
    if (agent) {
        ...
        if (controller) {
            if (controller->Activate(*agent, false)) {
                m_curAgentMode = mode;
                OnAgentSpawned(agent);
                UE_LOG(LogDeepDriveSimulation, Log, TEXT("Spawning agent controlled by %s"),
                       *(controller->getControllerName()));
            } else UE_LOG(LogDeepDriveSimulation, Log, TEXT("Couldn't activate controller %s"),
                          *(controller->getControllerName()));
        } else UE_LOG(LogDeepDriveSimulation, Log, TEXT("Couldn't spawn controller"));
    } else UE_LOG(LogDeepDriveSimulation, Log, TEXT("Couldn't spawn agent"));
...
void ADeepDriveSimulation::spawnAdditionalAgents() {
    for (auto &data : AdditionalAgents) {
        ...
        if (agent) {
            ...
            ADeepDriveAgentControllerBase *controller = spawnController(data.Mode, data.ConfigurationSlot,
                                                                        data.StartPositionSlot);
            if (controller) {
                if (controller->Activate(*agent, false)) {
                    OnAgentSpawned(agent);
                    UE_LOG(LogDeepDriveSimulation, Log, TEXT("Additional agent spawned, controlled by %s at %p"),
                           *(controller->getControllerName()), controller);
                } else UE_LOG(LogDeepDriveSimulation, Log, TEXT("Couldn't activate controller %s"),
                              *(controller->getControllerName()));
            } else UE_LOG(LogDeepDriveSimulation, Log, TEXT("Couldn't spawn controller for additional agent"));
        } else UE_LOG(LogDeepDriveSimulation, Log, TEXT("Couldn't spawn additional agent"));

    }
}

ADeepDriveAgentControllerBase *
ADeepDriveSimulation::spawnController(EDeepDriveAgentControlMode mode, int32 configSlot, int32 startPosSlot) {
...
UDeepDriveRandomStream *ADeepDriveSimulation::GetRandomStream(const FName &RandomStreamId) {
    return RandomStreams.Contains(RandomStreamId) ? RandomStreams[RandomStreamId].getRandomStream()
                                                  : m_DefaultRandomStream.getRandomStream();
}
...
int32 ADeepDriveSimulation::AddOneOffAgent(TSubclassOf<ADeepDriveAgent> AgentClass, const FTransform &Transform,
                                           bool BindToRoad) {
    int32 id = 0;
    if (OneOffAgentsTrack) {
        ...
        if (agent) {
            ...
            ADeepDriveAgentOneOffController *controller = Cast<ADeepDriveAgentOneOffController>(
                    GetWorld()->SpawnActor(ADeepDriveAgentOneOffController::StaticClass(), &Transform,
...
void ADeepDriveSimulation::SetOneOffAgentControlValue(int32 AgentId, float Steering, float Throttle, float Brake,
                                                      bool Handbrake) {
